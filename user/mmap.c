...
dirty_test(void)
{
  ...
  //Creates a mmap to the full size of the file.
  char *p = mmap(0, PGSIZE * 20, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  close(fd);

  printf("dirty_test: write the 11th page\n");
  for (int i = PGSIZE * 10; i < PGSIZE * 11; i++)
    p[i] = 'Z';

  printf("dirty_test: write the 2nd page\n");
  for (int i = PGSIZE; i < PGSIZE*2; i++)
    p[i] = 'R';

  //syscall to clear dirty counts
  cdw();
  //Expect just 2 pages of writeback, using dirty bit
  munmap(p, PGSIZE * 20);
  //syscall to read dirty writebacks
  uint64 dw = rdw();

  printf("# of Dirty writes: %ld\n", dw);
